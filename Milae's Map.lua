...
function MapGlobals:New()
	...
	mglobal.iceLatitude			= 75	-- Furthest down ice is allowed to go


	
	--Adjusting these will generate larger or smaller landmasses and features.
	mglobal.landMinScatter			= 0.02 	--Recommended range:[0.02 to 0.1]	--Higher values makes continental divisions and stringy features more likely,
	...
	mglobal.islePercent			= 0.06 -- Percent of coast tiles with an isle (atoll)
	...
	mglobal.pacificSize			= math.min(4, Round(mglobal.pacificSize * mapW))
	...
	if oRiftWidth == 1 then
		...
		mglobal.landPercent = mglobal.landPercent + 0.01 * oceans
	...
	elseif oRiftWidth == 3 then
		...
		mglobal.landPercent = mglobal.landPercent + 0.03 * oceans
	end	
	...
	--have need for them.
	mglobal.northAttenuationFactor	= 0.09
	mglobal.northAttenuationRange	= 0.13 --percent of the map height.
	mglobal.southAttenuationFactor	= 0.14
	mglobal.southAttenuationRange	= 0.16
...
function GetMapScriptInfo()
	...
	return {
		Name = "Milae's Map",
		Description = "Milae's modifications to the Communitas Map",
		IsAdvancedMap = 0,
...
function GetMapInitData(worldSize)
	...
	local worldsizes = {
		...
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = {53, 36},
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = {65, 44},
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = {73, 48},
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = {79, 53},
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = {87, 58}
		}
...
function GenerateTerrain()
	...
	local grassThreshold = rainfallMap:FindThresholdFromPercent(mg.grassPercent,false,true)
...
function GenerateIslands(rainfallMap)
	...
	local grassThresholdA = rainfallMap:FindThresholdFromPercent(mg.plainsPercent+0.01,false,true)
		...
		if Plot_IsWater(plot) then
			...
			if plot:IsMountain() then
				...
				elseif plotTerrainID == TerrainTypes.TERRAIN_GRASS then
					...
					if plotPercent.TERRAIN_PLAINS >= 0.4 * randPercent then
						plot:SetTerrainType(TerrainTypes.TERRAIN_PLAINS, true, true)
					end
				end
			end
...
function IsGoodMarshPlot(plot,wetThreshold,plainsThreshold,desertThreshold,rain,temp)
	...
	for nearPlot in Plot_GetPlotsInCircle(plot, 0, 1) do
		...
		if nearPlot:GetFeatureType() == FeatureTypes.FEATURE_MARSH then
				odds = odds + mg.clumpingModifier * mg.marshPercent
		end
...
function IsGoodJunglePlot(plot,wetThreshold,plainsThreshold,desertThreshold,rain,temp)
	...
	for nearPlot in Plot_GetPlotsInCircle(plot, 0, 2) do
		...
		if nearPlot:GetFeatureType() == FeatureTypes.FEATURE_JUNGLE then
			odds = odds + mg.clumpingModifier * mg.junglePercent
		end
	end
...
function IsGoodForestPlot(plot,wetThreshold,plainsThreshold,desertThreshold,rain,temp)
	...
	for nearPlot in Plot_GetPlotsInCircle(plot, 0, 2) do
		if nearPlot:GetTerrainType() == TerrainTypes.TERRAIN_DESERT or nearPlot:GetTerrainType() == TerrainTypes.TERRAIN_SNOW then
			odds = odds -  mg.forestPercent
		end
		if nearPlot:GetFeatureType() == FeatureTypes.FEATURE_JUNGLE then
			odds = odds + 0.4 * mg.clumpingModifier * mg.forestPercent
		elseif nearPlot:GetFeatureType() == FeatureTypes.FEATURE_FOREST then
			odds = odds + mg.clumpingModifier * mg.forestPercent
		end
	end
...
function AssignStartingPlots:GetMajorStrategicResourceQuantityValues()
	...
	local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 3, 3, 3, 4, 5, 3;
	-- Check the resource setting.
	if self.resource_setting == 1 then -- Sparse
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 2,2, 3,3, 4, 3;
	elseif self.resource_setting == 3 then -- Abundant
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 4, 4, 5, 5, 6, 4;
	end
...
--]]
	...
	self:ProcessResourceList(35, 1, self.grass_flat_no_feature, resources_to_place)
	self:ProcessResourceList(35, 1, self.plains_flat_no_feature, resources_to_place)
	self:ProcessResourceList(45, 1, self.desert_wheat_list, resources_to_place)
	self:ProcessResourceList(55, 1, self.tundra_flat_no_feature, resources_to_place)
	
	resources_to_place = {
		{self.iron_ID, iron_amt, 100, 0, 2}
	};
	self:ProcessResourceList(90, 1, self.hills_open_list, resources_to_place)
	self:ProcessResourceList(120, 1, self.flat_open_no_tundra_no_desert, resources_to_place)
	self:ProcessResourceList(55, 1, self.desert_flat_no_feature, resources_to_place)
	self:ProcessResourceList(55, 1, self.tundra_flat_no_feature, resources_to_place)
	
	resources_to_place = {
		{self.coal_ID, coal_amt, 100, 0, 2}
	};
	self:ProcessResourceList(90, 1, self.hills_open_list, resources_to_place)
	self:ProcessResourceList(120, 1, self.flat_open_no_tundra_no_desert, resources_to_place)
	self:ProcessResourceList(80, 1, self.desert_flat_no_feature, resources_to_place)
	self:ProcessResourceList(45, 1, self.tundra_flat_no_feature, resources_to_place)

	resources_to_place = {
		{self.oil_ID, oil_amt, 100,0, 1}
	};
	self:ProcessResourceList(20, 1, self.desert_flat_no_feature, resources_to_place)
	self:ProcessResourceList(35, 1, self.tundra_flat_no_feature, resources_to_place)
	self:ProcessResourceList(120, 1, self.flat_open_no_tundra_no_desert, resources_to_place)
	self:ProcessResourceList(25, 1, self.snow_flat_list, resources_to_place)
	self:ProcessResourceList(20, 1, self.marsh_list, resources_to_place)
	self:ProcessResourceList(30, 1, self.jungle_flat_list, resources_to_place)
	
	resources_to_place = {
		{self.aluminum_ID, alum_amt, 100, 1, 3}
	};
	self:ProcessResourceList(70, 1, self.hills_open_list, resources_to_place)
	self:ProcessResourceList(100, 1, self.flat_open_no_tundra_no_desert, resources_to_place)
	self:ProcessResourceList(70, 1, self.desert_flat_no_feature, resources_to_place)
	self:ProcessResourceList(60, 1, self.tundra_flat_no_feature, resources_to_place)

	
	resources_to_place = {
		{self.uranium_ID, uran_amt, 100, 1, 3}
	};
	self:ProcessResourceList(120, 1, self.hills_jungle_list, resources_to_place)
	self:ProcessResourceList(200, 1, self.hills_open_list, resources_to_place)
	self:ProcessResourceList(75, 1, self.tundra_flat_no_feature, resources_to_place)
	self:ProcessResourceList(120, 1, self.desert_flat_no_feature, resources_to_place)
	self:ProcessResourceList(300, 1, self.flat_open_no_tundra_no_desert, resources_to_place)
	
	resources_to_place = {
		{self.iron_ID, iron_amt, 25, 0, 2},
		{self.coal_ID, coal_amt, 40, 0, 2},
		{self.uranium_ID, uran_amt, 15, 1, 2},
		{self.aluminum_ID, alum_amt, 20, 1, 3}
	};
	self:ProcessResourceList(70, 1, self.hills_forest_list, resources_to_place)
	self:ProcessResourceList(90, 1, self.flat_forest_no_tundra, resources_to_place)
	self:ProcessResourceList(70, 1, self.tundra_flat_forest, resources_to_place)
	self:ProcessResourceList(25, 1, self.marsh_list, resources_to_place)
	self:ProcessResourceList(45, 1, self.jungle_flat_list, resources_to_place)
	
	
	resources_to_place = {
		{self.uranium_ID, uran_amt, 20, 1, 3},
		{self.iron_ID, iron_amt, 30, 0, 1},
		{self.coal_ID, coal_amt, 30, 0, 1},
		{self.aluminum_ID, alum_amt, 20, 0, 1}
	};
	self:ProcessResourceList(3, 1, self.snow_flat_list, resources_to_place)
	--self:ProcessResourceList(3, 1, self.snow_hills_list, resources_to_place) (doesn't work)
	end
	
	self:AddModernMinorStrategicsToCityStates() -- Added spring 2011
	
	self:PlaceSmallQuantitiesOfStrategics(24 * resMultiplier, self.land_list)
...
function AssignStartingPlots:PlaceBonusResources()
	...
	self:ProcessResourceList(12 * resMultiplier, 3, self.desert_wheat_list, resources_to_place)
	self:ProcessResourceList(16 * resMultiplier, 3, self.plains_flat_no_feature, resources_to_place)
	...
	resources_to_place = {
	{self.cow_ID, 1, 100, 0, 1} };
	self:ProcessResourceList(16 * resMultiplier, 3, self.grass_flat_no_feature, resources_to_place)

	
-- CBP
	resources_to_place = {
	{self.bison_ID, 1, 100, 0, 1} };
	self:ProcessResourceList(14 * resMultiplier, 3, self.plains_flat_no_feature, resources_to_place)
	self:ProcessResourceList(18 * resMultiplier, 3, self.tundra_flat_no_feature, resources_to_place)
-- END

	resources_to_place = {
	{self.sheep_ID, 1, 100, 0, 1} };
	self:ProcessResourceList(15 * resMultiplier, 3, self.hills_open_list, resources_to_place)
	self:ProcessResourceList(26 * resMultiplier, 3, self.tundra_hills_no_feature, resources_to_place)
	-- 13
